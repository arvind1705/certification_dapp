...
$(document).ready(function () {
    ...
    if (typeof web3 === "undefined" || !web3.currentProvider.isMetaMask) {
        $('#login-disabled-text').append('Metamask is not installed. Please Install Metamask. <a href="https://chrome.google.com/webstore/detail/metamask/nkbihfbeogaeaoehlefnkodbefgpgknn">aquí</a> y refresca la página.')
    ...
    if (web3.eth.coinbase == null) {
        $('#login-disabled-text').append('MetaMask extension account not detected. Please ensure an account is logged in with the metamask extension to enable services.')
        $('#login-disabled-text').show();
        $('#login-button').attr('disabled', 'disabled');
    }
...
function makeTransaction(fileHash) {
    ...
    certification.addCertificate(userToCertificate, fileHash,
        function (error, hash) {
            ...
            if (error) { 
                console.log(error); 
                alert("Theres some error in the blockchain. Please try again"); 
                $("#certificate-error-container").show();
                $("#error-text-feedback").html(error); 
            } else {
